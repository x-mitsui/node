...
const kIoDone = Symbol('kIoDone');//IO操作结束了
const kIsPerformingIO = Symbol('kIsPerformingIO');//是否正在进行IO操作

const kFs = Symbol('kFs');// 代表fs模块
const kHandle = Symbol('kHandle');// 句柄，文件描述符
...
function ReadStream(path, options) {
  ...
  if (options.autoDestroy === undefined) {
    options.autoDestroy = false;// 如果为假，需要手动关闭文件描述符
  }
...
// options说明：http://nodejs.cn/api/fs.html#fscreatewritestreampath-options
function WriteStream(path, options) {
  ...
  if (options.fd == null) {
    this.fd = null;
    this[kFs] = options.fs || fs;// fs.js内部调用的jsbindings
    validateFunction(this[kFs].open, 'options.fs.open');
  ...
  if (!this[kFs].write) {// 到这里我们就可以知道，如果options为空，那么真正的write函数在fs里
    this._write = null;
  }
  if (!this[kFs].writev) {
    this._writev = null;
...
WriteStream.prototype._write = function(data, encoding, cb) {
  this[kIsPerformingIO] = true;// 进行io操作
  // 参数依次是文件描述符，数据，数据所取起点，数据所取长度，目标写入位置，回调
  this[kFs].write(this.fd, data, 0, data.length, this.pos, (er, bytes) => {
    this[kIsPerformingIO] = false;// io操作结束
    if (this.destroyed) {
      // Tell ._destroy() that it's safe to close the fd now.
      cb(er);
  ...
  if (this.pos !== undefined)
    this.pos += data.length;//得到下一次写入位置
};
